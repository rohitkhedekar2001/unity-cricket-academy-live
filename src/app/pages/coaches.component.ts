...
import { coachDesignations, Coach, CoachDesignation } from '../models/app.models';
  ...
  template: `
    <section class="space-y-5">
      ...
      <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <article *ngFor="let coach of coaches()" class="panel p-4 transition hover:-translate-y-0.5 hover:shadow-lg">
    ...
    <div *ngIf="formOpen()" class="fixed inset-0 z-40 overflow-auto bg-black/55 p-4">
      <form class="mx-auto my-6 max-w-2xl rounded-lg bg-white p-5 shadow-2xl" [formGroup]="form" (ngSubmit)="save()">
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <label><span class="form-label">Designation</span><select class="form-input mt-1" formControlName="designation"><option *ngFor="let item of designations" [value]="item">{{ item }}</option></select></label>
...
export class CoachesComponent implements OnInit {
  ...
  readonly designations = coachDesignations;
  readonly form = this.fb.group({
    ...
    designation: ['AssistantCoachLevel1' as CoachDesignation, Validators.required],
    has_admin_access: [false]
  });

  ngOnInit(): Promise<void> { return this.load(); }
  load(): Promise<void> { return this.data.listCoaches().then((rows) => this.coaches.set(rows)); }
    ...
    try {
      if (this.editingId()) {
        await this.data.updateCoachAccount({
          ...
          p_designation: value.designation!
        });
      } else {
